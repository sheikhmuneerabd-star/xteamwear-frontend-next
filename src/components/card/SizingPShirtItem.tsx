...
export default function SizingPShirtItem({ shirt, handleClick }: SizingPShirtItemProps) {
  ...
  const handleVariantClick = (e: React.MouseEvent<HTMLDivElement>, variant: ProductVariant) => {
    e.preventDefault();
    e.stopPropagation();
    setActive(variant);
  };
  ...
  return (
    <Link href={`/card/${shirt.id}/${encodeURIComponent(active.color)}`} onClick={handleClick}>
