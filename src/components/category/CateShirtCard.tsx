...
export default function CateShirtCard({ shirt, grid }: CateShirtCardProps) {
  ...
  return (
    <Link href={`/card/${shirt.id}/${encodeURIComponent(active.color)}`} className="shirt2 block w-[99%]">
      <div className="group">
        <div className={grid === 1 ? "flex" : "block"}>
          <div
            className={`cursor-pointer group/img group-hover:-translate-y-2 transition-all duration-300 relative overflow-hidden aspect-[4/5] ${
                grid === 1 ? "w-[220px] shrink-0" : "w-full"
            }`}
            >
            <Image
              src={active.images[0]}
          ...
          <div className={grid === 1 ? "ml-4 w-[73%]" : "md:pt-4 md:pl-4 md:pr-4 pt-3 pl-2 pb-0"}>
            ...
            <p className={`text-sm md:text-base lg:text-base text-gray-700 max-[320px]:hidden ${grid === 1 ? "block" : "hidden"}`}>
